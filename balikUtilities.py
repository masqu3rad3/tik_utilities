...
def getControllers(mode="phonemes"):
    ...
    try:
        ...
        return None
    if pm.nodeType(selectionSet)=="objectSet":
        controllers = sorted(selectionSet.members())
    else:
        controllers = selectionSet
    return controllers
...
def lipSynch(word, seperation=2, useSelectedRange=True):
    ...
    contBalikMaster = getControllers("curve8")
    ...
    for i in range(len(word)):
        ...
        if command != "default" and command != "repeat":
            ...
            if command != phenomeDict[word[i - 1]]:
                pm.setKeyframe(contBalikMaster, at=command, v=0, t=targetFrame - seperation, itt="flat", ott="flat")
                # put a keyframe for the lipsync
            pm.setKeyframe(contBalikMaster, at=command, v=1, t=targetFrame, itt="flat", ott="flat")
